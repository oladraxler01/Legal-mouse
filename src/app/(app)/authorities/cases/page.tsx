...
export default function CasesAuthorityPage() {
  ...
  const availableCategories = Array.from(
    new Set(cases.map(c => c.sub_type).filter(Boolean))
  ).sort();
  const categories = ["All", ...availableCategories];

  const filteredCases = cases.filter((c) => {
    const matchesSearch = c.title.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesCategory = activeCategory === "All" || c.sub_type === activeCategory;
    return matchesSearch && matchesCategory;
  });

  // Group filtered cases by sub_type
  const groupedCases = filteredCases.reduce((acc, c) => {
    const category = c.sub_type || 'Uncategorized';
    if (!acc[category]) {
      acc[category] = [];
    }
    acc[category].push(c);
    return acc;
  }, {} as Record<string, CaseAuthority[]>);

  // Sort groups alphabetically
  const sortedGroups = Object.keys(groupedCases).sort();
    ...
    <div className="min-h-screen bg-surface text-on-surface p-6 md:p-12 pb-40 transition-colors duration-300">
      ...
      <header className="mb-10 animate-in fade-in duration-500">
        ...
        <p className="font-body text-on-surface-variant text-lg">
          Explore foundational precedents and pivotal court rulings.
        </p>
      </header>
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-10 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
        ...
        <div className="relative w-full lg:w-1/3">
          ...
          <input
            ...
            placeholder="Search cases (e.g., 'Donoghue v Stevenson')"
      ...
      <div className="animate-in fade-in slide-in-from-bottom-8 duration-500 delay-200">
        {loading ? (
            ...
            {sortedGroups.map((group) => (
              <div key={group} className="space-y-4">
                <h2 className="font-serif text-2xl font-bold text-on-surface border-b border-outline-variant/10 pb-2">
                  {group.includes('/') ? group.replace('/', ' - ') : group}
                </h2>
                <div className="bg-surface-container border border-outline-variant/10 rounded-2xl overflow-hidden shadow-sm">
                  <div className="divide-y divide-outline-variant/10">
                    {groupedCases[group].map((c) => (
                      <div
                        key={c.id}
                        className="w-full flex flex-col md:flex-row md:items-center justify-between p-5 hover:bg-surface-container-high transition-colors group gap-4 md:gap-0"
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="font-label text-xs text-on-surface-variant font-medium uppercase tracking-wider mt-1">
                              {c.sub_type ? c.sub_type.split('/')[0] : 'Law'}
                            </span>
                          </div>
                        </div>
          ...
          <div className="bg-surface-container border border-outline-variant/10 rounded-2xl p-16 text-center flex flex-col items-center justify-center border-dashed">
            ...
            <h3 className="font-serif text-2xl font-bold mb-3 text-on-surface">No Cases Found</h3>
            <p className="font-body text-on-surface-variant max-w-sm mx-auto">
              {searchQuery 
                ? `We couldn't find any results matching "${searchQuery}".`
                : "More case materials coming soon for this category."}
            </p>
            {searchQuery && (
              <button 
